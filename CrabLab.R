...
#Baseline take TWO !!!! making a line graph. To do this, the datasheet has to be 
...
#Normalized by weight; blanks are divided by the weight of the amount of water, 
...
#Running into the issue here that the EGCs are so weighty that they make the 
#fluor/gram quite low (like ~4 or 5), so if we were to do this, we'd need a 
#way to convert fluor to O2. 

#OOOOH! or we could just do changes in metabolic rate. So take the slope of each
#species at each temp, and do the change. So if the hemis here are 56/whatever
#and in the hot treatment they're like 100/whatever that's like a ~79% increase in 
#metabolic rate. Much to consider... 


#need to bootstrap the green crabs because of difference in sample size.
#alternatively, we compare means of change in respiration just between each 
#genus to avoid the need to bootstrap 
#i no no wanna bootstrap :(

BaselineTest <- read.csv(file = "Baseline_noBLank_test.csv", header = T)

sum.baselineTest <- BaselineTest %>%
  group_by(Genus) %>%
  summarise(
    N = n(),
    mean = mean(Percent.change),
...
t.test(sum.baselineTest$mean)

anovaTest <- aov(mean ~ Genus*temp, data = sum.baseline)
summary(anovaTest)
TukeyHSD(anovaTest)

anovaTake2 <- aov(mean ~ as.factor(Genus), data = sum.baseline)
summary(anovaTake2)
TukeyHSD(anovaTake2, conf.level = 0.90)
